...
const SemesterCard: React.FC<SemesterCardProps> = ({
  ...
  if (semesterKey === "semester7" || semesterKey === "semester8") {
    if (semesterData.informationSystemSecurity?.courses) {
      allCourses.push(...semesterData.informationSystemSecurity.courses);
    } else if (semesterData.networkSecurityEngineering?.courses) {
      allCourses.push(...semesterData.networkSecurityEngineering.courses);
    } else if (semesterData.secureSoftwareTechnology?.courses) {
      allCourses.push(...semesterData.secureSoftwareTechnology.courses);
    }
  }
  ...
  return (
    <div className="relative group">
      ...
      <div className="relative overflow-hidden border-0 shadow-lg sm:shadow-xl bg-background-surface rounded-xl sm:rounded-2xl backdrop-blur-sm">
        {/* Header */}
        <div
          className="relative transition-all duration-500 cursor-pointer md:p-2 xs:p-3 sm:p-4 md:p-5 lg:p-6 bg-gradient-to-br from-blue-600 via-purple-600 to-indigo-700 hover:from-blue-700 hover:via-purple-700 hover:to-indigo-800"
          onClick={onToggle}
        >
          {/* Decorative elements */}
            ...
            <div className="flex items-center space-x-2 xs:space-x-3 sm:space-x-4 md:space-x-5 lg:space-x-6">
              ...
              <div>
                <h3 className="mb-0.5 xs:mb-1 sm:mb-1.5 md:mb-2 text-sm xs:text-base sm:text-lg md:text-xl lg:text-2xl font-bold text-white">
                  {t(`programInfo.${semesterKey}.title`)}
                </h3>
                <div className="flex items-center space-x-1.5 xs:space-x-2 sm:space-x-3 md:space-x-4 text-[10px] xs:text-xs sm:text-sm md:text-base text-blue-100">
                  <div className="flex items-center space-x-1 xs:space-x-1.5 sm:space-x-2">
                    ...
                    <span>
                      {totalCourses} {t("programInfoDetails.courses")}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center space-x-1 xs:space-x-1.5 sm:space-x-2">
                    ...
                    <span>
                      {totalCredits} {t("programInfoDetails.credits")}
                    </span>
                  </div>
                </div>
              </div>
        ...
        {isExpanded && (
          <div className="p-2 xs:p-3 sm:p-4 md:p-5 lg:p-6 bg-gradient-to-br from-gray-50 to-blue-50/30">
                  ...
                  <div className="p-1 border-0 border-l-4 border-red-500 rounded-lg shadow-md lg:p-3 sm:rounded-xl bg-gradient-to-br from-red-50 to-pink-100">
                    ...
                    <div className="grid grid-cols-2 gap-2 2 sm:grid-cols-3 md:grid-cols-4 xs:gap-3 sm:gap-4 md:gap-5">
                      {semesterData.compulsory.courses.map((course, index) => (
                        <CourseCard
                          key={`${semesterKey}-compulsory-${index}`}
                          course={course}
                        />
                      ))}
                    </div>
            ...
            {/* Specializations */}
            {semesterData.informationSystemSecurity && (
              <div className="mt-3 xs:mt-4 sm:mt-5 md:mt-6">
                <div className="p-2 border-0 border-l-4 border-purple-500 rounded-lg shadow-md xs:p-3 sm:p-4 md:p-5 sm:rounded-xl bg-gradient-to-br from-purple-50 to-indigo-100">
                  <h4 className="flex items-center mb-2 text-xs font-bold text-purple-800 xs:mb-3 sm:mb-4 md:mb-5 xs:text-sm sm:text-base md:text-lg">
                    <div className="p-1 xs:p-1.5 sm:p-2 md:p-2.5 mr-2 xs:mr-3 rounded-lg bg-purple-500/20">
                      <Code className="w-2.5 h-2.5 xs:w-3 xs:h-3 sm:w-3.5 sm:h-3.5 md:w-4 md:h-4 text-purple-600" />
                    </div>
                    {semesterData.informationSystemSecurity.title}
                  </h4>
                  <div className="grid grid-cols-2 gap-2 2 sm:grid-cols-3 md:grid-cols-4 xs:gap-3 sm:gap-4 md:gap-5">
                    {semesterData.informationSystemSecurity.courses.map(
                      (course, index) => (
                        <CourseCard
                          key={`${semesterKey}-iss-${index}`}
                          course={course}
                        />
                      )
                    )}
            ...
            {semesterData.networkSecurityEngineering && (
              <div className="mt-3 xs:mt-4 sm:mt-5 md:mt-6">
                <div className="p-2 border-0 border-l-4 border-green-500 rounded-lg shadow-md xs:p-3 sm:p-4 md:p-5 sm:rounded-xl bg-gradient-to-br from-green-50 to-emerald-100">
                  <h4 className="flex items-center mb-2 text-xs font-bold text-green-800 xs:mb-3 sm:mb-4 md:mb-5 xs:text-sm sm:text-base md:text-lg">
                    <div className="p-1 xs:p-1.5 sm:p-2 md:p-2.5 mr-2 xs:mr-3 rounded-lg bg-green-500/20">
                      <Code className="w-2.5 h-2.5 xs:w-3 xs:h-3 sm:w-3.5 sm:h-3.5 md:w-4 md:h-4 text-green-600" />
                    </div>
                    {semesterData.networkSecurityEngineering.title}
                  </h4>
                  <div className="grid grid-cols-2 gap-2 2 sm:grid-cols-3 md:grid-cols-4 xs:gap-3 sm:gap-4 md:gap-5">
                    {semesterData.networkSecurityEngineering.courses.map(
                      (course, index) => (
                        <CourseCard
                          key={`${semesterKey}-nse-${index}`}
                          course={course}
                        />
                      )
                    )}
            ...
            {semesterData.secureSoftwareTechnology && (
              <div className="mt-3 xs:mt-4 sm:mt-5 md:mt-6">
                <div className="p-2 border-0 border-l-4 border-blue-500 rounded-lg shadow-md xs:p-3 sm:p-4 md:p-5 sm:rounded-xl bg-gradient-to-br from-blue-50 to-indigo-100">
                  <h4 className="flex items-center mb-2 text-xs font-bold text-blue-800 xs:mb-3 sm:mb-4 md:mb-5 xs:text-sm sm:text-base md:text-lg">
                    <div className="p-1 xs:p-1.5 sm:p-2 md:p-2.5 mr-2 xs:mr-3 rounded-lg bg-blue-500/20">
                      <Code className="w-2.5 h-2.5 xs:w-3 xs:h-3 sm:w-3.5 sm:h-3.5 md:w-4 md:h-4 text-blue-600" />
                    </div>
                    {semesterData.secureSoftwareTechnology.title}
                  </h4>
                  <div className="grid grid-cols-2 gap-2 2 sm:grid-cols-3 md:grid-cols-4 xs:gap-3 sm:gap-4 md:gap-5">
                    {semesterData.secureSoftwareTechnology.courses.map(
                      (course, index) => (
                        <CourseCard
                          key={`${semesterKey}-sst-${index}`}
                          course={course}
                        />
                      )
                    )}
