...
const CareerProspects = () => {
  ...
  const getCareerIcon = (index: number) => {
    const icons = [Wifi, Network, Cpu, Shield, Radio, Satellite];
  ...
  const careerPaths = [
    "telecomEngineer",
    "networkCommunicationSpecialist",
    "electronicsCircuitDesignEngineer",
    "secureCommunicationEngineer",
    "iotWirelessCommunicationSpecialist",
    "satelliteRadioCommunicationEngineer",
  ];

  const careers = careerPaths.map((path) => {
    const career = t(`careerPathsTelecom.${path}`, {
      returnObjects: true,
    }) as CareerPath;
    return career;
  });
    ...
    <div className="max-w-6xl min-h-screen p-4 mx-auto md:p-6 bg-background-surface">
      ...
      <div className="mb-8 text-center md:mb-12">
        ...
        <h3 className="mb-3 text-xl font-bold md:mb-4 md:text-2xl text-text-primary">
          {t("careerPathsTelecom.title")}
        </h3>
        <p className="mb-4 text-text-secondary">
          {t("careerPathsTelecom.intro")}
        </p>
